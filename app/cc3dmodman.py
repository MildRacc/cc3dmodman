...
# BASE_DIR is where the runtime content lives relative to the script or exe
if getattr(sys, "frozen", False):
    # Running as a PyInstaller exe
    BASE_DIR = os.path.dirname(sys.executable)
else:
    # Running from source (app folder)
    BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))

# If frozen by PyInstaller, sys.executable points to the exe location
APP_NAME = "CC3DModMan"
MODS_DIR = os.path.join(BASE_DIR, "mods")
CC3DMODMAN_DIR = os.path.join(BASE_DIR, "runtime")
TOOLS_DIR = os.path.join(BASE_DIR, "runtime/tools")

if os.name == "nt":
    GODOTPCKTOOL_PATH = os.path.join(TOOLS_DIR, "godotpcktool.exe")
else:
    GODOTPCKTOOL_PATH = os.path.join(TOOLS_DIR, "godotpcktool")
    ...
    def inject_and_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if modded_executable.endswith(".exe") and os.name == "nt":
                ...
                subprocess.run(
                    [modded_executable],
                    cwd=os.path.dirname(self.executable_path),  # run in the original game folder
                    env=os.environ
                )
